...
public class RadiusPacket {
	...
	public Map<String,String> toMap() {
		...
		map.put("packetType",getPacketTypeName());
		map.put("packetId",String.valueOf(packetIdentifier));
		for (Iterator i = attributes.iterator(); i.hasNext();) {
			RadiusAttribute attr = (RadiusAttribute) i.next();
			map.put(attr.getAttributeTypeObject().getName().replaceAll("-","_"),attr.getAttributeValue());
		}
		return map;
	}
