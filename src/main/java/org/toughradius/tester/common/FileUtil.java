package org.toughradius.tester.common;
...
//        }
    ...
    public static String tailFile(String filename,int readLength,String split) throws IOException {
        RandomAccessFile rf = null;
        StringBuilder buff = new StringBuilder();
        rf = new RandomAccessFile(filename, "r");
        long total = rf.length();
        long pos = total<readLength? 0 : total - readLength;
        rf.seek(pos);
    ...
    public static final boolean writeFile(String filename, String content) {
        try {
            BufferedWriter fos = new BufferedWriter(new FileWriter(new File(filename)));
            fos.write(content);
            fos.close();
            return true;
        } catch (IOException ie) {
